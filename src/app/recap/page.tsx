            ...
            <div className="space-y-4">
                    ...
                    {userItems && userItems.length > 0 && (
                        <div className="flex flex-wrap gap-2 mt-3">
                            {userItems.slice(0, 5).map((ui: any) => (
                                <Link key={ui.id} href={`/items/${ui.items?.id}`} className="text-xs px-2 py-1 bg-white/5 border border-white/10 rounded-full text-zinc-300 hover:border-amber-500/30 hover:text-amber-400 transition-colors">
                                    {ui.items?.name}
                                </Link>
                            ))}
                            {userItems.length > 5 && (
                ...
                {longestItem && (
                    <div className="bg-black/40 border border-white/10 rounded-3xl p-6">
                        ...
                        <Link href={`/items/${(longestItem.items as any)?.id}`} className="flex items-center gap-4 hover:opacity-80 transition-opacity">
